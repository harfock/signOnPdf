...
function openExportedInNewTab(){
  ...
  popup.document.open();
  popup.document.write(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${safeName}</title><style>html,body{margin:0;height:100%;font-family:Arial,sans-serif;background:#525659;color:#fff}body{display:flex;flex-direction:column}.bar{height:52px;background:#fff;color:#222;display:flex;align-items:center;gap:10px;padding:0 12px;box-sizing:border-box;flex-shrink:0}.name{font-weight:600;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.btn{border:1px solid #ccc;background:#fff;border-radius:7px;padding:8px 14px;text-decoration:none;color:#222;font-size:14px}.pages{flex:1;overflow:auto;padding:18px;display:flex;flex-direction:column;align-items:center;gap:18px;box-sizing:border-box}.page{background:#fff;box-shadow:0 3px 12px #0008;max-width:100%}.page canvas{display:block;max-width:100%;height:auto}</style></head><body><div class="bar"><div class="name">${safeName}</div><a class="btn" id="download" download="${safeName}" href="${viewerUrl}">Save PDF</a></div><div class="pages" id="pages"><div>Loading PDF…</div></div><script type="module">import * as pdfjsLib from "https://cdnjs.cloudflare.com/ajax/libs/pdf.js/4.10.38/pdf.min.mjs";pdfjsLib.GlobalWorkerOptions.workerSrc="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/4.10.38/pdf.worker.min.mjs";const url=${JSON.stringify(viewerUrl)};const pages=document.getElementById('pages');try{const doc=await pdfjsLib.getDocument({url}).promise;pages.innerHTML='';for(let i=1;i<=doc.numPages;i++){const page=await doc.getPage(i);const base=page.getViewport({scale:1});const availableWidth=Math.max(280,pages.clientWidth-36);const scale=Math.min(1.5,Math.max(.5,availableWidth/base.width));const vp=page.getViewport({scale});const wrap=document.createElement('div');wrap.className='page';wrap.style.width=vp.width+'px';wrap.style.maxWidth='100%';const canvas=document.createElement('canvas');canvas.width=Math.ceil(vp.width);canvas.height=Math.ceil(vp.height);canvas.style.width='100%';canvas.style.height='auto';canvas.style.aspectRatio=`${base.width} / ${base.height}`;canvas.style.objectFit='contain';wrap.appendChild(canvas);pages.appendChild(wrap);await page.render({canvasContext:canvas.getContext('2d',{alpha:false}),viewport:vp}).promise;}}catch(e){console.error(e);pages.textContent='Unable to preview PDF. Use Save PDF.';}</script></body></html>`);
