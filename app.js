...
const textDialog=$("textDialog"),textInput=$("textInput"),textDialogTitle=$("textDialogTitle"),textCancel=$("textCancel"),textConfirm=$("textConfirm"),homeLogo=$("homeLogo");
...
const PEN_STYLES={fine:{width:1.5,cap:"round",join:"round"},ballpoint:{width:2.4,cap:"round",join:"round"},marker:{width:5.2,cap:"round",join:"round"},brush:{width:5.8,cap:"round",join:"round"}};
...
function drawFreehandStroke(points,last=true){
  ...
  const opts=brush?{
    ...
    easing:t=>t,
    start:{cap:true,taper:0.35},
    end:{cap:true,taper:0.45},
    last
  }:{
    size:st.width,
    thinning:0.05,
...
function renderSavedSignatures(){const list=getSavedSignatures();savedSignatures.innerHTML="";if(!list.length){savedSignatures.innerHTML=`<div class="empty-signatures">${tr("noSavedSignatures")}</div>`;return}const title=document.createElement("div");title.className="signature-label";title.textContent=tr("reuseSignature");savedSignatures.appendChild(title);const grid=document.createElement("div");grid.className="signature-grid";list.forEach(data=>{const wrap=document.createElement("div");wrap.className="saved-signature-wrap";const b=document.createElement("button");b.type="button";b.className="saved-signature";const img=document.createElement("img");img.src=data;img.alt="Saved signature";b.appendChild(img);b.onclick=()=>useSignature(data);const del=document.createElement("button");del.type="button";del.className="saved-signature-delete";del.textContent="×";del.title=tr("removeSavedSignature");del.setAttribute("aria-label",tr("removeSavedSignature"));del.onclick=e=>{e.preventDefault();e.stopPropagation();removeSavedSignature(data)};wrap.appendChild(b);wrap.appendChild(del);grid.appendChild(wrap)});savedSignatures.appendChild(grid)}
function removeSavedSignature(data){const list=getSavedSignatures().filter(x=>x!==data);localStorage.setItem("pdfSignerSignatures",JSON.stringify(list));renderSavedSignatures()}
function clearSig(){resizeSig()}
function useSignature(data){commit();const l=currentList(),w=180,h=70,pos=newAnnotationPosition(w,h);l.push({id:newId(),type:"signature",data,x:pos.x,y:pos.y,w,h,fontSize:0,aspectRatio:2.5});setCurrentList(l);selectedId=l.at(-1).id;signatureDialog.classList.add("hidden");renderPage()}
...
function goHome(){try{if(exportedUrl){URL.revokeObjectURL(exportedUrl)}}catch{}exportedUrl=null;exportedBlob=null;pdfDocument=null;sourcePdfBytes=null;sourceFileName="document";currentPage=1;zoom=1;selectedId=null;annotations=new Map();history=[];renderVersion++;editor.classList.add("hidden");welcome.classList.remove("hidden");pdfContainer.replaceChildren();if(exportResult)exportResult.classList.add("hidden");updateControls()}
...
el.addEventListener("pointermove",e=>{if(!mode)return;if(mode==="move"){a.x=Math.max(0,ax+(e.clientX-sx)/zoom);a.y=Math.max(0,ay+(e.clientY-sy)/zoom)}else{a.w=Math.max(45,aw+(e.clientX-sx)/zoom);if(a.type==="signature"){const ratio=a.aspectRatio||((aw>0&&ah>0)?aw/ah:2.5);a.h=Math.max(24,a.w/ratio)}else{a.h=Math.max(24,ah+(e.clientY-sy)/zoom)}}el.style.left=`${a.x*zoom}px`;el.style.top=`${a.y*zoom}px`;el.style.width=`${a.w*zoom}px`;el.style.height=`${a.h*zoom}px`;if(a.type!=="signature")el.style.fontSize=`${a.fontSize*zoom}px`});
